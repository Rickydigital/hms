            ...
            <div class="sticky-top" style="top: 20px;">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <small class="text-muted d-block mt-2">Filters recent queue instantly</small>
                    </div>
                </div>
                ...
                <div class="card">
                    <div
                        class="card-header bg-success text-white d-flex justify-content-between align-items-center rounded-top">
                        <h5 class="mb-0">
                            Recent Queue (Last 3 Days)
                            <span class="badge bg-light text-success ms-2">{{ $todayVisits->count() }}</span>
                        </h5>
                        <small>{{ now()->subDays(2)->format('d M') }} - {{ now()->format('d M Y') }}</small>
                    </div>
                    ...
                    <div class="list-group list-group-flush" style="max-height: 70vh; overflow-y: auto;" id="queueList">
                            ...
                            <div class="d-flex justify-content-between align-items-start gap-3">
                                <div>
                                    <div class="fw-bold">
                                        ...
                                        <span class="text-muted">• {{ \Carbon\Carbon::parse($v->visit_date)->format('d M
                                            Y') }}</span>
                                        <span class="text-muted">• {{ $v->visit_time->format('h:i A') }}</span>
                                    </div>
                                ...
                                <span
                                    class="badge bg-{{ $v->status == 'consulting' ? 'warning' : ($v->status == 'sent_to_lab' ? 'info' : 'primary') }}">
                        ...
                        <div class="text-center py-5 text-muted">
                            No patients in the last 3 days<br>
                            <small>Queue will appear here once visits are created.</small>
                        </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="card-header bg-gradient-primary text-white py-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="mb-0 opacity-90">Token #{{ $visit->id }} • {{ $visit->created_at->format('d M Y •
                                h:i A') }}</p>
                        </div>
                        <span class="badge bg-white text-primary fs-5 px-4 py-2">{{ ucfirst(str_replace('_', ' ',
                            $visit->status)) }}</span>
                    </div>
                </div>
                ...
                <div class="card-body p-4 p-xl-5">
                        ...
                        <div class="col-xl-7">
                            <div class="card h-100">
                                <div class="card-body">
                                    ...
                                    <p class="text-muted mb-3">{{ $visit->patient->age }} years • {{
                                        $visit->patient->gender }}</p>
                                    <div class="row g-3">
                                        <div class="col-sm-6"><strong>Phone:</strong> {{ $visit->patient->phone }}</div>
                                        <div class="col-sm-6"><strong>Valid Till:</strong> {{
                                            $visit->patient->expiry_date->format('d M Y') }}</div>
                                        <div class="col-12"><strong>Address:</strong> {{ $visit->patient->address ?: '—'
                                            }}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card">
                                <div class="card-body text-center">
                                    ...
                                    <div class="row row-cols-3 g-3">
                                        <div>
                                            <div class="vital-badge bg-danger">BP<br><small>{{ $visit->vitals?->bp ?:
                                                    '—' }}</small></div>
                                        </div>
                                        <div>
                                            <div class="vital-badge bg-warning">Pulse<br><small>{{
                                                    $visit->vitals?->pulse ?: '—' }}</small></div>
                                        </div>
                                        <div>
                                            <div class="vital-badge bg-info">Temp<br><small>{{
                                                    $visit->vitals?->temperature ?: '—' }}°F</small></div>
                                        </div>
                                        <div>
                                            <div class="vital-badge bg-success">Weight<br><small>{{
                                                    $visit->vitals?->weight ?: '—' }} kg</small></div>
                                        </div>
                                        <div>
                                            <div class="vital-badge bg-purple">Height<br><small>{{
                                                    $visit->vitals?->height ?: '—' }} cm</small></div>
                                        </div>
                                        <div>
                                            <div class="vital-badge bg-primary">RR<br><small>{{
                                                    $visit->vitals?->respiration ?: '—' }}</small></div>
                                        </div>
                                    </div>
                    ...
                    <div class="card mb-5 border-0 shadow-lg rounded-4">
                        <div class="card-header bg-gradient-success text-white py-4 rounded-top-4">
                            <h5 class="mb-0">Lab Results • {{ $visit->labOrders->where('is_completed', true)->count() }}
                                Completed</h5>
                        </div>
                        <div class="card-body p-4">
                            <div class="row g-4">
                                ...
                                <div class="col-md-6 col-lg-4">
                                    <div
                                        class="border rounded-4 p-4 shadow-sm h-100 {{ $order->result->is_abnormal ? 'border-danger bg-danger-subtle' : 'border-success bg-light' }}">
                                        ...
                                        <div class="mt-3">
                                            ...
                                            <div class="fs-4 fw-bold text-dark">
                                                ...
                                                <small class="text-muted fs-6">({{ $order->result->normal_range ?? '—'
                                                    }})</small>
                                            </div>
                                            @elseif($order->result->result_text)
                                            <div class="alert alert-info py-2 px-3 mb-0">{{ $order->result->result_text
                                                }}</div>
                                            @endif
                                            @if($order->result->remarks)
                                            <small class="text-muted d-block mt-2"><strong>Remark:</strong> {{
                                                $order->result->remarks }}</small>
                                            @endif
                    ...
                    <div
                        class="card mb-5 border-0 shadow-sm {{ $visit->labOrders->where('is_completed', false)->where('is_paid', false)->count() > 0 ? 'border-danger' : 'border-warning' }}">
                        <div
                            class="card-header {{ $visit->labOrders->where('is_completed', false)->where('is_paid', false)->count() > 0 ? 'bg-danger' : 'bg-warning' }} text-white py-3">
                            <h6 class="mb-0 d-flex justify-content-between align-items-center">
                                Pending Lab Tests ({{ $visit->labOrders->where('is_completed', false)->count() }})
                                @if($visit->labOrders->where('is_completed', false)->where('is_paid', false)->count() >
                                0)
                        ...
                        <div class="card-body p-4">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <div
                                        class="d-flex align-items-center p-3 rounded-3 {{ $order->is_paid ? 'bg-light' : 'bg-danger bg-opacity-10' }} border-start {{ $order->is_paid ? 'border-success' : 'border-danger' }} border-5">
                            ...
                            <div class="alert alert-danger mt-4 mb-0">
                                <strong>Important:</strong> Patient must pay at the billing counter before lab can
                                process these tests.
                            </div>
                            @endif
                        </div>
                    ...
                    <div class="card mb-5 border-0 shadow-lg rounded-4">
                        <div class="card-header bg-gradient-purple text-white py-4 rounded-top-4">
                            <h5 class="mb-0">Completed Procedures • {{ $visit->procedureOrders->where('is_issued',
                                true)->count() }} Issued</h5>
                        </div>
                        <div class="card-body p-4">
                            <div class="row g-4">
                                ...
                                <div class="col-md-6 col-lg-4">
                                    <div
                                        class="border rounded-4 p-4 shadow-sm {{ $order->is_paid && $order->is_issued ? 'border-success bg-light' : 'border-warning bg-warning-subtle' }}">
                                        <div class="d-flex justify-content-between align-items-start mb-3">
                                            <h6 class="fw-bold text-purple mb-0">{{ $order->procedure->procedure_name }}
                                            </h6>
                                            @if($order->is_paid)
                                        ...
                                        <div>
                                            @if($order->extra_instruction)
                                            <small class="text-info d-block mb-2">{{ $order->extra_instruction
                                                }}</small>
                                            @endif
                    ...
                    <div
                        class="card mb-5 border-0 shadow-sm {{ $visit->procedureOrders->where('is_issued', false)->where('is_paid', false)->count() > 0 ? 'border-danger' : 'border-warning' }}">
                        <div
                            class="card-header {{ $visit->procedureOrders->where('is_issued', false)->where('is_paid', false)->count() > 0 ? 'bg-danger' : 'bg-warning' }} text-white py-3">
                            <h6 class="mb-0 d-flex justify-content-between align-items-center">
                                Pending Procedures ({{ $visit->procedureOrders->where('is_issued', false)->count() }})
                                @if($visit->procedureOrders->where('is_issued', false)->where('is_paid', false)->count()
                                > 0)
                                <span class="badge bg-white text-danger fs-6">Payment Required</span>
                                @else
                        ...
                        <div class="card-body p-4">
                            ...
                            0)
                            <div class="alert alert-danger mt-4 mb-0">
                                <strong>Important:</strong> Patient must pay at the billing counter before procedure can
                                be issued.
                            </div>
                            @endif
                        </div>
                    ...
                    <form action="{{ route('doctor.prescription.store', $visit) }}" method="POST" id="prescriptionForm">
                                ...
                                <div class="row g-4">
                                    <div class="col-md-4">
                                        ...
                                        <textarea name="chief_complaint" rows="4"
                                            class="form-control">{{ $visit->vitals?->chief_complaint }}</textarea>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label text-info fw-bold">Examination</label>
                                        <textarea name="examination" rows="4"
                                            class="form-control">{{ $visit->vitals?->examination }}</textarea>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label text-success fw-bold">Diagnosis</label>
                                        <textarea name="diagnosis" rows="4"
                                            class="form-control">{{ $visit->vitals?->diagnosis }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div id="labContainer">
                                            <template id="labTemplate">
                                                <div
                                                    class="prescription-item position-relative mb-3 p-3 bg-light rounded-3 border">
                                                    <button type="button"
                                                        class="remove-prescription position-absolute top-0 end-0 btn btn-sm btn-danger rounded-circle m-2">×</button>

                                                    <select name="lab_tests[]"
                                                        class="form-select form-select-lg select2-lab"
                                                        multiple="multiple" style="width: 100%;">
                                                        <option value="">Search lab tests...</option>
                                                        @foreach(\App\Models\LabTestMaster::active()->orderBy('test_name')->get()
                                                        as $test)
                                                        <option value="{{ $test->id }}" data-price="{{ $test->price }}">
                                                            {{ $test->test_name }} — {{ number_format($test->price) }}
                                                            Tsh
                                                        </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <button type="button" id="addLabTest"
                                            class="btn btn-info rounded-pill px-5 py-3 mt-3 shadow">
                                            Add Lab Test Group
                                        </button>

                                        <textarea name="lab_instruction" class="form-control mt-3" rows="2"
                                ...
                                <div class="card">
                                    <div class="card-body">
                                            ...
                                            <template id="procedureTemplate">
                                                <div
                                                    ...
                                                    <select name="procedures[]"
                                                        class="form-select form-select-lg select2-procedure"
                                                        multiple="multiple" style="width: 100%;">
                                                        <option value="">Search procedures...</option>
                                                        @foreach(\App\Models\ProcedureMaster::active()->orderBy('procedure_name')->get()
                                                        as $proc)
                                                        <option value="{{ $proc->id }}" data-price="{{ $proc->price }}">
                                                            {{ $proc->procedure_name }} — {{ number_format($proc->price)
                                                            }} Tsh
                                                        </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <button type="button" id="addProcedure"
                                            class="btn btn-purple rounded-pill px-5 py-3 mt-3 shadow">
                                            Add Procedure
                                        </button>

                                        <textarea name="procedure_instruction" class="form-control mt-3" rows="2"
                                            placeholder="Special instruction for procedures..."></textarea>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div id="medicineContainer">
                                            <template id="medicineTemplate">
                                                <div
                                                    class="item-card position-relative mb-4 p-5 rounded-4 border border-primary border-opacity-10">
                                                    <button type="button"
                                                        class="remove-item btn btn-danger rounded-circle shadow-sm">×</button>

                                                    <div class="mb-4">
                                                        <label class="form-label fw-bold text-primary fs-5">Medicine
                                                            Name</label>
                                                        <select name="medicines[medicine_id][]"
                                                            class="form-select form-select-lg select2-medicine"
                                                            style="width:100%;">
                                                            ...
                                                            <option value="{{ $m->id }}"
                                                                ...
                                                                @if($m->current_stock <= 10) <span
                                                                    class="text-danger fw-bold"> (Low Stock!)</span>
                                                                    @endif
                                                            </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="mb-4">
                                                        ...
                                                        <input type="text" name="medicines[dosage][]"
                                                            class="form-control form-control-lg text-center fw-bold fs-4"
                                                            placeholder="e.g. 1-0-1 or 1 tab twice daily">
                                                    </div>

                                                    <div class="mb-4">
                                                        <label class="form-label fw-bold text-warning">Duration
                                                            (Days)</label>
                                                        <input type="number" name="medicines[duration_days][]" min="1"
                                                            class="form-control form-control-lg text-center fw-bold fs-4"
                                                            placeholder="5">
                                                    </div>

                                                    <div class="mb-4">
                                                        <label class="form-label fw-bold text-purple">Frequency /
                                                            Timing</label>
                                                        <input type="text" name="medicines[instruction][]"
                                                            class="form-control form-control-lg"
                                                            placeholder="e.g. After food, Before sleep, SOS, With water...">
                                                    </div>

                                                    <div class="mt-4">
                                                        <label class="form-label text-success fw-bold">Additional
                                                            Instruction (Optional)</label>
                                                        <textarea name="medicines[extra_instruction][]"
                                        ...
                                        <button type="button" id="addMedicineRow"
                                            class="btn btn-success rounded-pill px-5 py-3 mt-3 shadow">
                                            Add Medicine
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <select name="injection_medicine_id" id="injection_medicine_id"
                                        ...
                                        <input type="text" name="injection_route" id="injection_route"
                                            class="form-control form-control-lg"
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <select name="ward_id" id="ward_id" class="form-select form-select-lg mb-3">
                                            ...
                                            <option value="{{ $ward->id }}">{{ $ward->ward_name }} • {{
                                                $ward->available_beds }} free</option>
                                            @endforeach
                                        </select>
                                        <textarea name="admission_reason" id="admission_reason" class="form-control"
                                            rows="3" placeholder="Reason for admission..." readonly></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-center mt-5">
                            <button type="submit" name="follow_up_only" value="1"
                                class="btn btn-warning btn-lg px-5 py-3 rounded-pill me-4 shadow">
                                Follow-up Only
                            </button>
