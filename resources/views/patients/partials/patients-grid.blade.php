...
<div class="col-12 col-sm-6 col-lg-4 col-xl-3 patient-card"
    ...
    <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden position-relative hover-lift">
        ...
        <div class="position-absolute top-0 end-0 p-2 z-3">
            @if($patient->is_rch)
                <span class="badge bg-info rounded-pill px-3 py-2 small fw-medium">RCH</span>
            @endif
        </div>
        ...
        <div class="card-body p-4 d-flex flex-column text-center">
            <div class="avatar-lg mx-auto mb-3 bg-soft-primary rounded-circle d-flex align-items-center justify-content-center">
                <i class="bi {{ $patient->gender == 'Male' ? 'bi-person' : 'bi-person-fill' }} text-primary"
            ...
            @php
                $recentVisits = $patient->visits()->where('created_at', '>=', now()->subHours(72));

                $hasActiveRecentVisit = (clone $recentVisits)
                    ->whereIn('status', [
                        'in_opd',
                        'consulting',
                        'sent_to_lab',
                ...
                $hasPendingLabPayment = (clone $recentVisits)
                    ->whereHas('labOrders', function ($q) {
            ...
            <div class="d-flex gap-2 justify-content-center mt-auto flex-wrap">
                <button type="button"
                        ...
                        onclick='showPatient(@json($patient->append(["age_display"])))'>
                    View
                </button>
                ...
                <button type="button"
                        ...
                        onclick='openEditModal(@json($patient))'>
                    <i class="bi bi-pencil"></i> Edit
                </button>

                @if($patient->is_active && !$patient->isExpired())

                    @if($hasPendingLabPayment)
                        <span class="btn btn-secondary btn-sm rounded-pill disabled">
                            Pending Lab Payment
                        </span>

                    @elseif($patient->is_rch)
                        <button type="button"
                                class="btn btn-primary btn-sm rounded-pill"
                                data-bs-toggle="modal"
                                data-bs-target="#assignLabModal"
                                onclick="openAssignLabModal({{ $patient->id }}, @js($patient->name), @js($patient->patient_id))">
                            Assign Lab
                        </button>

                    @else
                        @if(!$hasActiveRecentVisit)
                            <button type="button"
                                    class="btn btn-success btn-sm rounded-pill"
                                    data-bs-toggle="modal"
                                    data-bs-target="#createVisitModal"
                                    onclick="openVisitModal({{ $patient->id }}, @js($patient->name), @js($patient->patient_id))">
                                Start Visit
                            </button>
                        @else
