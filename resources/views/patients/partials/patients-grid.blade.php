...
<div class="col-12 col-sm-6 col-lg-4 col-xl-3 patient-card" data-patient-name="{{ strtolower($patient->name) }}"
    ...
    <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden position-relative hover-lift">
        ...
        <div class="card-body p-4 d-flex flex-column text-center">
            <div
                class="avatar-lg mx-auto mb-3 bg-soft-primary rounded-circle d-flex align-items-center justify-content-center">
            ...
            <div class="d-flex gap-2 justify-content-center mt-auto flex-wrap">
                <button type="button" class="btn btn-outline-info btn-sm rounded-pill" data-bs-toggle="modal"
                    data-bs-target="#viewPatientModal" onclick='showPatient({!! json_encode($patient->append(['
                    age_display'])) !!})'>
                    View
                </button>

                <!-- Edit Button -->
                <button type="button" class="btn btn-outline-warning btn-sm rounded-pill" data-bs-toggle="modal"
                    data-bs-target="#editPatientModal" onclick='openEditModal({!! json_encode($patient) !!})'>
                    <i class="bi bi-pencil"></i> Edit
                </button>

                @if($patient->is_active && !$patient->isExpired())
                @php
                $hasActiveRecentVisit = $patient->visits()
